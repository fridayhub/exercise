...
#define UMCR  0xB8 /* UART RS-485 Mode Control Register LJH add*/
...
#define UMCR_TXB8    (1<<2) //Transmit RS-485 bit 8 (the ninth bit or 9 th bit)
#define UMCR_SADEN   (1<<3) //RS-485 Slave Address Detected Interrupt Enable
#define TANDA_SPACE 0x5468 //LJH add
#define TANDA_MARK  0x5469

static int tandaFalg = 0;
//end
...
static struct of_device_id imx_uart_dt_ids[] = {
	...
	{ .compatible = "fsl,imx6ul-485", .data = &imx_uart_devdata[IMX6Q_UART], },
...
static irqreturn_t imx_rxint(int irq, void *dev_id)
{
	...
	while (readl(sport->port.membase + USR2) & USR2_RDR) {
        ...
        if(tandaFalg == 1 && (rx & URXD_PRERR));
        else if (unlikely(rx & URXD_ERR)) {
            if (rx & URXD_BRK)
...
static irqreturn_t imx_int(int irq, void *dev_id)
{
	...
	unsigned int sts;
	unsigned int sts2;
	unsigned int tmp;  //LJH add


	tmp = readl(sport->port.membase + USR2);
...
static void tanda_485_set(struct imx_port *sport)
{
    unsigned int ucr2;
    unsigned int umcr;
	ucr2 = readl(sport->port.membase + USR2);
	ucr2 |= UCR2_PREN;
    writel(ucr2, sport->port.membase + USR2);
    
    umcr = readl(sport->port.membase + UMCR);
    umcr |= UMCR_MDEN | UMCR_TXB8;
    umcr &= ~UMCR_SLAM;
		   ...
		   struct ktermios *old)
{
    ...
    if(termios->c_cflag & CMSPAR) { //LJH add
        tandaFalg = 1;
    //    tanda_485_set(sport);
    }
	...
	}else if (sport->rs485.flags & SER_RS485_ENABLED)  /*LJH begin*/
        /*disable transmitter*/
        if (!(sport->rs485.flags & SER_RS485_RTS_AFTER_SEND))
            ucr2 |= UCR2_CTS;    /*LJH end*/
...
static int serial_imx_probe(struct platform_device *pdev)
{
    ...
    sport->port.rs485_config = imx_rs485_config;  /*LJH add two lines*/
    sport->rs485.flags = SER_RS485_RTS_ON_SEND | SER_RS485_RX_DURING_TX;
