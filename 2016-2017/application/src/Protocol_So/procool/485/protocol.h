...
#define MAIN_VERSION 0X02 //不能更改
#define SUB_VERSION  0X0A //用户版本号 目前为2
#define REMOTE_RETRY_TIMES		3	//远程控制帧重发次数
/***********************控制单元命令定义*******************************************************/
...
typedef struct thingWorxData
{
	...
	unsigned int deviceAddr[2];
	unsigned int componType;			//部件类型
	unsigned int componLoopID[2];		//部件回路地址
    unsigned int componAddrs[2];      	//部件地址
	unsigned int channelNum;         	//通道号
	unsigned int componStatus[2];		//部件状态
	unsigned char occuTime[6];			//发生时间
	int AuthorizationStatus;			//授权状态
    unsigned int AuthorizationId[12];	//系统ID
	int AuthorizationControlType;		//控制类型
	int indicate;						//控制指示
    unsigned int deviceOperType;      //设备操作类型	
