...
typedef struct thingWorxData
{
	...
	unsigned int  ControllerID; 	//控制器ID  	30
	unsigned int ControllerType; 	//控制器类型	29
	unsigned int componType;		//部件类型
	unsigned int componaddr[8];	//部件地址	8字节低字节在前
	unsigned int componStatus;		//部件状态
	unsigned char occuTime[6];		//发生时间
	unsigned int  servicetype;		//调用服务名
	unsigned char OperatorMark;		//操作员类型
	unsigned char OperatorPersonID;	//操作员编号
}twData;
